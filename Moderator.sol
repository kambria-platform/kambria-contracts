...
contract Moderator is Ownable {
  ...
  function Moderator() public {
    MAX_CANDIDATES = 100;
  }

  function changeMaxCandidates(uint256 maxcan) public onlyOwner {
    require(maxcan > 0);
  ...
  function newBallot(uint256 rate, bytes32[] cids) public onlyOwner {
  ...
  function endBallot(uint256 sbid) public onlyOwner {
    require(sbid >= 0 && sbid < ballots.length);
    bool success = ballots[sbid].call(keccak256("end()"));
    EndBallot(msg.sender, ballots[sbid], success);
  }

  // sbid: Sequence Ballot Identity
  function restartBallot(uint256 sbid) public onlyOwner {
    require(sbid >= 0 && sbid < ballots.length);
    bool success = ballots[sbid].call(keccak256("restart()"));
    RestartBallot(msg.sender, ballots[sbid], success);
  }
}